...
- **Steering** : a real-time, **configurable widget dashboard** (Apple-style edit mode with drag-to-reorder, per-widget sizing from 1/4 to full width, and an add/remove gallery, persisted per user) covering overall compliance, KPI indicators, compliance by framework, active objectives, priority risks, upcoming deadlines, the current-to-residual risk treatment flow chart and risk matrices; a unified To do / Doing / Done "Tasks" board aggregating action plans, treatment actions, audits and risk assessments; ISO 27001 management reviews; and PDF/DOCX/PPTX report generation (SoA, audit report, risk register, meeting minutes)
