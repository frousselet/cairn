...
BRIEFING_PROMPT = (
    ...
    "START of the paragraph (immediately after <p>, before any text or <b>), "
    "never in the middle or at the end, and never more than one per paragraph. "
    "Choose an emoji that fits that paragraph's subject and VARY them - do not "
    "reuse the same emoji from one paragraph or one day to the next (for example "
    "a magnifier or clipboard for audits, a warning sign for critical risks, a "
    "chart or check mark for compliance). Use only professional, "
    "workplace-appropriate emojis with no sexual, ambiguous or offensive "
    "connotation. "
    ...
    "loi, 'regulation' = une reglementation, 'contract' = un contrat, "
    "'internal_policy' = une politique interne, 'industry_framework' = un "
    "referentiel sectoriel, 'other' = un referentiel. This type is only an "
    "indication to help you phrase things naturally WHEN USEFUL (e.g. 'la norme "
    "ISO 27001' or 'le reglement RGPD'); you are NOT required to state it, and "
    "you must NEVER append it as a parenthetical tag like 'ISO 27001 (norme)'. "
    "List each distinct standard only once, never repeating the same one), the "
    "lead auditor (use the exact full name from "
    ...
    "'a partial scope' - always say which scopes. Give each audit's period from "
    "its start_date to its end_date (e.g. 'du 12 au 20 juin 2026'). When an "
    "audit carries a "
    "'progress' object, also report where it stands: how many requirements have "
    "been audited so far (requirements_audited of requirements_total) and the "
    "compliance rate over the requirements already audited (compliance_rate_pct, "
    "a percentage), then its findings - mentioning only the non-zero ones among "
    "major_non_conformities, minor_non_conformities, observations and "
    "improvement_opportunities, each named with its exact entity (e.g. '2 "
    "non-conformites majeures, 5 mineures, 3 observations et 1 opportunite "
    "d'amelioration'). Avoid repetition: when audits "
