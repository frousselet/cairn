"""Data migration: add Trust Center permissions and assign them to system groups.
...
# (feature, human label, actions)
_FEATURES = [
    ("settings", "Settings", ["read", "update"]),
    ("certification", "Certifications", ["create", "read", "update", "delete", "approve"]),
    ("subprocessor", "Subprocessors", ["create", "read", "update", "delete", "approve"]),
    ("measure", "Measures", ["create", "read", "update", "delete", "approve"]),
    ("document", "Documents", ["create", "read", "update", "delete", "approve"]),
    ("document_request", "Document requests", ["read", "approve", "delete"]),
]
...
NEW_PERMISSIONS = [
    {
        "codename": f"trust_center.{feature}.{action}",
        "name": f"Trust Center - {flabel} - {_ACTION_LABELS[action]}",
        "module": "trust_center",
        "feature": feature,
        "action": action,
    }
    for feature, flabel, actions in _FEATURES
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("accounts", "0041_add_assistant_feedback_permission"),
    ]
