...
| `is_applicable` | boolean | required, default `true` | Applicable to the scope. Derived automatically from the associated risks (read-only) when the framework has `applicability_managed_by_risks` enabled: see `docs/specs/m3-compliance/framework.md` |
