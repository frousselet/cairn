...
- **Essential & support asset tables adopt the two-line cell pattern**: on both asset lists, the **Type** column was folded under **Category** (category over type, muted), the **Owner** became a 32px people-cell (avatar + name over job title), and the three **C / I / A** (DIC) columns were merged into a single column showing the three labelled colour badges in a row. Essential assets additionally show the data classification and a "Personal data" flag (muted) under the name. Support assets fold the **End of life** date under the **Status** column (muted, with the EOL badge), keeping the end-of-life danger row highlight. This condenses the essential-assets table from eleven to eight columns and the support-assets table from thirteen to nine.
