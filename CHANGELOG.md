...
- **Legacy duplicate `status` column dropped on every entity whose status merely mirrored `workflow_state`** (essential & support assets, contracts, certificates, action plans, compliance assessments, management reviews, risks, risk assessments, treatment plans, acceptances, vulnerabilities and the six EBIOS deliverables). The lifecycle step (`workflow_state`) is now the single source of truth; the `sync_legacy_status` bridge is gone. `obj.status` (read and write) and `obj.get_status_display()` keep working as thin aliases over `workflow_state` (via `LegacyStatusMixin`), so call sites and the `?status=` API/UI filters are unaffected, and the state is exposed read-only in the serializers. The create/edit forms no longer carry a `status` selector (state is driven by the lifecycle stepper).
- **Separate approval axis removed entirely.** The `is_approved` / `approved_by` / `approved_at` fields are dropped from every `BaseModel` subclass (migrations across `assets`, `compliance`, `context`, `reports`, `risks`, `trust_center`), and with them the whole approval apparatus: the `ApprovalContextMixin` / `ApprovableUpdateMixin` view mixins, the `ApprovableAPIMixin` (renamed to `LifecycleAPIMixin`, its `approve` / `reject` DRF actions gone), the generic `approve_*` MCP tools and the approve-on-update reset logic, the `EntryKind.APPROVAL` history event, the per-detail-page "Approved by / Approval date / Approve" UI blocks, and the risk register's bulk **Approve** action. Validation is now a single concept: reaching a lifecycle step that counts in reports (e.g. the default lifecycle's **Validated**), gated by the per-transition permission on the lifecycle engine. Domain approvals that are genuine lifecycle actions are kept (Trust Center document-request approve/reject; the SWOT `validate` action now lands the analysis in its **Validated** lifecycle step and stamps `validated_by` / `validated_at`).
- **`VersioningConfig` admin screen and model deleted.** The per-model "Versioning" configuration (approval-enabled toggle, major-change fields, workflow-name override) was a control surface for the retired approval / first-generation-workflow machinery and had no remaining readers once those were gone. The model, its `core/config/` CRUD views, URLs, form, templates, the Administration > General > **Versioning** sidebar entry and its tests are removed (migration `core.0005_delete_versioningconfig`). Lifecycle assignment is declared on the model itself (`LIFECYCLE_NAME`), so no runtime configuration table is needed.
...
- **Every lifecycle graph is now bookended by a generic Draft entry and Archived exit, and rendered with the directed-graph stepper (the scope / supplier look).** All lifecycles moved off the plain wrapping-pill (`line`) stepper onto the schema-driven directed-graph renderer, so the risk, treatment-plan, EBIOS, action-plan, management-review, asset and trust-center flows now read as connected graphs with arrows, numbered stages, loop-back arcs and a detached Archived exit - consistent with Scopes and Suppliers. The port helper (`lifecycle_from_state_flags`) prepends a generic **Draft** step and appends a generic **Archived** step whenever the domain states do not already provide them, and auto-wires `draft -> <initial state>`, `any -> archived` and `archived -> draft` (restore). New elements therefore start in Draft and can be archived from any state; the domain's former initial / terminal states (identified, closed, planned, ...) become ordinary steps that keep their governance flags.
- **Full migration to the standardised lifecycle engine (`core.lifecycle`), decommissioning the first-generation `core.workflow` engine.** Every domain element now runs a `Lifecycle` instead of a `Workflow`: a generic 4-step **default lifecycle** (Draft -> Pending validation -> Validated -> Archived) backs every model without a specific one, and the per-app specific machines (assets, compliance, risks incl. EBIOS, reports, trust center) are ported to lifecycles that keep the exact same step codes, labels and governance flags (no `workflow_state` data migration). Each detail page renders the lifecycle stepper (`LifecycleStepperMixin`); the generic / bespoke transition endpoints, the DRF `.../transition/` action and the MCP `transition_*` tools route through the lifecycle service, enforcing the per-transition permission (the default lifecycle keeps `.approve` on Validate / Archive; the trust center keeps its publishing permission) via the engine. A new-element creation hook snaps `workflow_state` to its lifecycle's initial step; the lifecycle stepper now prompts for a comment on comment-required transitions.
...
- **Lifecycle graph steppers no longer overlap or clip their nodes at any screen size.** The edge classifier now recognises a send-back-to-Draft as a loop (not a spurious forward branch), which was stacking the Draft / Pending / Validated nodes on top of each other, and it draws the Archive exit even when the archive transition has a specific source. Wide graphs (many branch outcomes) on narrow screens scroll horizontally as a safety net instead of clipping, while the snake layout keeps most graphs fitting without a scrollbar.
- First-run onboarding no longer crashes its background schedulers against a database that the onboarding migrations have not built yet. On a fresh instance the SPOF auto-detection scheduler (`assets`) and the semantic-index startup rebuild (`assistant`) used to start at server boot and immediately query tables (`assets_assetdependency`, `assistant_semanticindex`, ...) that did not exist yet, spamming tracebacks and contending with the in-flight migration (which could stall the progress bar). Both jobs now gate their database access on a new `core.onboarding.state.instance_ready()` (schema migrated **and** first user created) and stay completely off the database while an onboarding migration/seed is running, so on SQLite (the VS Code debug stack) they no longer fight the migration's write lock. The Docker image build was also fixed: `autobahn` (a transitive `daphne` dependency) refuses to emit a pure-Python wheel when its optional NVX C extension cannot compile, so the `Dockerfile` sets `AUTOBAHN_USE_NVX=0` for the dependency install.
