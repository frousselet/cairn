...
- **Rich-text editor replaced (Jodit -> pell)**: the buggy Jodit WYSIWYG is replaced by **pell**, a ~3.5 KB dependency-free editor. It is dressed in the frosted form-field language (glass card that surfaces on focus, transparent toolbar of Bootstrap-Icons buttons, fading hairline) and emits **clean, semantic, attribute-free HTML** (`<b>`, `<i>`, `<u>`, `<s>`, `<ul>`/`<ol>`, `<a href>`, `<h2>`/`<h3>`, `<blockquote>`, `<hr>`) that passes the public-rendering sanitizer (which strips every attribute except `<a href>`) and renders correctly anywhere. Toolbar: bold, italic, underline, strikethrough, heading, subheading, quote, bulleted / numbered list, link, divider, clear formatting (tooltips translated). The editor auto-upgrades every `textarea.form-control`; opt out with `.no-richtext` (the former `.no-jodit` still works). Active-format highlighting on the toolbar, empty-content normalization (no phantom value for required-field detection), and a submit-time sync safety net are included. Stored content is **sanitized with DOMPurify before being seeded into the editor** (the formatting-tag whitelist matches the server-side sanitizer), closing the stored-XSS path of injecting HTML into a rich-text field and having it run when another user opens the edit form.
- **Multi-step form navigation moved outside the modal**: stepped create / edit modals lose their footer. Navigation is now two **floating round arrows just outside the dialog**, vertically centred (sharing the frosted button chrome): left goes back (hidden on the first step), right advances and, on the **last step, becomes a navy save check**. Clicking the check **spins it in place** while the item saves. There is no Cancel button : cancellation is the header X or a backdrop click. `Enter` advances / submits instead of submitting prematurely. On narrow screens the arrows tuck into the lower corners. Single-step forms keep the standard footer (Cancel left, Save right). When the step changes the modal **morphs its height with a smooth 260 ms ease** (measure-swap-measure height tween, skipped under `prefers-reduced-motion`) instead of snapping. (Supersedes the brief constant-height experiment, now removed.)
- **Modals aligned with the frosted system**: the modal **backdrop is now identical to the command palette / search overlay** - a page-tinted translucent veil (`color-mix(--bg-page 82%, transparent)`) with a soft `blur(4px)` instead of Bootstrap's dark dim - and the modal surface carries the **same hairline border and soft shadow as the buttons** (`1px --border-light`, `--shadow-md`, down from `--shadow-lg`/`--shadow-xl`). Applies to every Bootstrap modal (the shared form drawer `#itemDrawer`, confirm-deletion dialogs, the About modal). Documented in the brand guidelines.
- **Buttons redesigned as a solid frosted system**: every button now shares the **frosted chrome of the sidebar search field** (a `backdrop-filter` blur, a 1px hairline border, a soft shadow and a calm hover that lifts the shadow) and carries its meaning through a **solid fill instead of an outline / wireframe**. **Primary** is the navy identity colour (white label, with a hint of real frost); the **neutral / secondary** button is the search field's light glass itself (the single style for Cancel, Back, Export and other non-semantic actions); **danger / warning / success / info** fill with their semantic hue. Fill and label colours are AA-tuned per theme through new `--btn-*-bg` / `--btn-*-fg` tokens (and a deeper-tinted label in dark mode where the hues are vivid). Inside tables, action buttons stay calm and borderless (no fill until hover, colour kept on the icon, not a block). Bootstrap's `btn-outline-*` class names are remapped to the same solid fill as their `btn-*` counterparts, so existing markup keeps working; `.btn-link` stays a plain text link. **Button placement is standardized**: in modal and confirmation footers the neutral **Cancel** sits on the left and the primary / destructive action on the right, and Cancel is always neutral (it was previously styled red). The styleguide gains a Buttons section and the brand guidelines document the new doctrine.
...
- **Dependency graph cascade highlight**: clicking a node in the dependency graph spotlights its **entire dependency chain** - every node and edge reachable upstream (what it depends on) and downstream (what depends on it), walked transitively in both directions - and dims everything else, with the selected node ringed in the navy identity colour. Clicking the node again, or the background, clears the selection.
...
- **Scope tree field layout / Firefox checkbox**: the scope-selection field (and any multi-select checkbox widget) was rendered with Bootstrap's single-checkbox `.form-check` layout because `CheckboxSelectMultiple.input_type` is `'checkbox'`. That indented the field (`padding-left: 1.5em`, so it was narrower than sibling fields and the label dropped below the control) and applied `.form-check .form-check-input { float: left; margin-left: -1.5em }` to every tree checkbox, which in Firefox pulled the box under the collapse chevron and made the first scope unselectable. The shared field renderer now uses the `.form-check` layout only for *single* boolean checkboxes (`not widget.allow_multiple_selected`); multi-select widgets get the normal label-above, full-width layout. The scope field now matches the other fields' width and every checkbox is clickable.
