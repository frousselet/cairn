...
- **Search overlay entrance animation**: opening the search from the sidebar field or the topbar button morphs the dialog from the trigger's position and shape to its centered resting place (FLIP transform, 320 ms ease-out-expo). Ctrl+K keeps the plain fade and `prefers-reduced-motion` disables the flight.
