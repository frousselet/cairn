...
- **Essential & support asset tables adopt the two-line cell pattern**: on both asset lists, the **Owner** became a 32px people-cell (avatar + name over job title), and the three **C / I / A** (DIC) columns were merged into a single compact column showing the three colour badges in a row (order given by the header, with a tooltip). Essential assets show the **category and type** (`Category • Type`) under the name, dropping the standalone Category column. Support assets fold the **Type** under **Category** (category over type, muted) and the **End of life** date under the **Status** column (muted, with the EOL badge), keeping the end-of-life danger row highlight. This condenses the essential-assets table from eleven to seven columns and the support-assets table from thirteen to nine.
