...
- **Richer users admin table**: the Users administration list now shows a larger profile photo (48px, doubled) and stacks information on two lines with contrasting type shades : the **Name** column shows the display name (bold, accent) over the email (muted), and the **Job title** column shows the job title over the department (muted). The table is condensed from seven to five columns and vertically centred, for a calmer, more scannable layout. Client-side search still matches on email and department; sortable columns are Name, Status and Last login (the standalone Email-sort column was folded into Name).
