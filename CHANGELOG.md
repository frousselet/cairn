...
- **Context forms migrated to the modal engine (batch 1)**: the Issue, Activity and SWOT analysis create / edit forms are split into `Create` / `Update` subclasses over a shared stepped base, each with two declarative steps and a helper on every field; their modal templates are reduced to the header icon. FR translations added.
