...
- **French UI partly in English under the VS Code / mise dev setup**: the `stack: bootstrap` task (the `preLaunchTask` run on every `F5`) only migrated and seeded the database, never compiling the translation catalogs, so the `.mo` files were missing or stale and the interface rendered as a French/English mix. Bootstrap now runs `compilemessages` first (non-fatal if the `gettext` `msgfmt` binary is absent, with a hint), a dedicated **stack: compile messages** task is available for on-demand recompilation, and the installation guide documents the `gettext` prerequisite and the `compilemessages` step.
