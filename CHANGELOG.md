...
- **Multi-step forms keep a constant height**: create / edit modals with several steps **no longer grow or shrink when navigating** between steps - the body is fixed to the tallest step so Back / Next never make the dialog jump. CSS-only: the steps are stacked in a single grid cell (`.mform-steps`) and inactive steps stay in layout (`visibility:hidden` instead of `display:none`) so they keep sizing the cell; the step-navigation JS is unchanged.
