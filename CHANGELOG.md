...
- **Full migration to the standardised lifecycle engine (`core.lifecycle`), decommissioning the first-generation `core.workflow` engine.** Every domain element now runs a `Lifecycle` (a registered `core.lifecycle.Lifecycle`) instead of a `core.workflow.Workflow`: a generic 4-step **default lifecycle** (Draft -> Pending validation -> Validated -> Archived) backs every model without a specific one, and the per-app specific machines (assets, compliance, risks incl. EBIOS, reports, trust center) are ported to lifecycles that keep the exact same step codes, labels and governance flags (no `workflow_state` data migration). A new-element creation hook snaps `workflow_state` to its lifecycle's initial step. _(In progress across this release: foundation landed.)_
