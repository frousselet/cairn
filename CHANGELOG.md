...
- **Risk register table adopts the two-line cell pattern**: the **Name** now shows the originating assessment reference and the risk source (`ASS-x • source`) on a muted sub-line, folding away the standalone Assessment and Source columns; the current risk level is now shown **inside the priority badge** (`Priority · level`), folding away the Current level column; and the **Owner** became a 32px people-cell (avatar + name over job title). The bulk-select checkbox, Treatment, Status, Tags and Actions columns are unchanged. This condenses the register from eleven to eight data columns.
